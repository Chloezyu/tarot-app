...
// ============ STYLES ============
const fonts = `
@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;0,600;1,300;1,400&family=Noto+Serif+SC:wght@300;400;500;600&family=DM+Sans:wght@300;400;500&display=swap');
`;

// ============ CARD BACK COMPONENT ============
function CardBack({ number, selected, onClick, small, revealed }) {
  return (
    <div
      onClick={onClick}
      style={{
...
// ============ CARD FACE COMPONENT ============
function CardFace({ card, size = "medium", onClick }) {
  const sizes = {
    small: { w: 80, h: 120, titleSize: 11, symSize: 24, numSize: 9 },
    medium: { w: 140, h: 210, titleSize: 14, symSize: 40, numSize: 11 },
    large: { w: 220, h: 330, titleSize: 18, symSize: 56, numSize: 13 },
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        height: s.h,
        borderRadius: 10,
        background: "linear-gradient(175deg, #FAF6F0, #F0EBE1, #E8E0D4)",
        border: "1.5px solid rgba(180,165,140,0.4)",
        display: "flex",
    ...
    >
      ...
      <div style={{
        ...
        width: "30%",
        height: 1,
        background: `linear-gradient(90deg, transparent, ${suitColor}88, transparent)`,
      }} />
    </div>
  );
}
...
// ============ FLIP CARD ============
function FlipCard({ card, flipped, delay = 0, isReversed }) {
...
function HomePage({ onNavigate }) {
  const [phase, setPhase] = useState("welcome"); // welcome, shuffling, selecting, result
  ...
  const shuffle = useCallback(() => {
    ...
    deck.forEach((card, i) => {
      card._displayNum = i + 1;
    });
    // Random reversals
    const rev = {};
    deck.forEach((card) => {
      rev[card.id] = Math.random() > 0.5;
    });
    setReversals(rev);
      ...
      {phase === "shuffling" && (
        <div style={{
          ...
          <div style={{
            display: "flex",
            gap: 8,
            animation: "shuffleCards 1.8s ease-in-out",
          }}>
            {[1, 2, 3, 4, 5].map((n) => (
              <div
                key={n}
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{
                      fontFamily: "'DM Sans', sans-serif",
                      fontSize: 12,
                      color: isRev ? "#B87D6B" : "#8BA88B",
                      marginBottom: 12,
                      letterSpacing: "0.1em",
                    }}>
                      {isRev ? "逆位 REVERSED" : "正位 UPRIGHT"}
                    </div>
                    <div style={{
                      display: "flex",
                      gap: 8,
...
// ============ LIBRARY PAGE ============
function LibraryPage({ onCardSelect }) {
